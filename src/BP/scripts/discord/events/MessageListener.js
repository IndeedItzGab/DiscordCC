...
export async function MessageListener(data) {
  if(data.channel_id !== config.main.relayChannel || data.author.bot || !config.alerts.discordChat) return;
  
  // Assigning color name and unicode role
  let prefixName;
  if(guilds.get(data.guild_id).owner_id === data.author.id) {
    prefixName = `§e `
  } else if(data.member?.roles.some(r => config.main.moderatorRoles.includes(r))) {
    prefixName = `§d `
  } else {
    prefixName = `§3`
  }
  ...
  if(data.message_reference?.message_id) {
    const request = new HttpRequest(
        `https://discord.com/api/v10/channels/${config.main.relayChannel}/messages/${data.message_reference.message_id}`
    );
    request.method = HttpRequestMethod.Get;
    request.headers = [
        new HttpHeader(
            "Authorization",
            `Bot ${config.main.botToken}`
        )
    ];
